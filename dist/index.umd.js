...
var DatePicker = /** @class */ (function () {
    ...
    DatePicker.prototype.setYear = function (evt) {
        ...
        var lastMonth = new Date(selectedYear, 11, 1);
        this.yearView = !this.yearView;
        this.monthDays = this.generateDays(this.date);
        var lastMonthDays = this.generateDays(lastMonth);
        var first = this.monthDays[0].filter(function (item) { return item.day; })[0].day;
        var lasweek = lastMonthDays[lastMonthDays.length - 1].filter(function (item) { return item.day; });
        var last = lasweek[lasweek.length - 1].day;
    ...
    DatePicker.prototype.setMonth = function (evt) {
        if (evt.target.getAttribute('id')) {
            ...
            var monthDays = this.monthDays = this.generateDays(this.date);
            if (this.settings.onlyMonth) {
                if (this.settings.closeOnSelect) {
                    this.popover = false;
                }
            ...
            var first = monthDays[0].filter(function (item) { return item.day; })[0].day;
            var lasweek = monthDays[monthDays.length - 1].filter(function (item) { return item.day; });
            var last = lasweek[lasweek.length - 1].day;
            if (!this.settings.isLast) {
                this.date.setDate(first);
            }
